...
class TINYIMAGENET_BLUR30(Dataset):
    def __init__(self,
        root = "./dataset/TinyImagenet",
        ...
        if mode in ["test"]:
            for sid in range(10):
                collection_name = os.path.join(prefix,"tinyimagenet200_test_rand1_cls20_task%d.json"%(sid))
...
class TINYIMAGENET_DISJOINT(Dataset):
    def __init__(self,
        root = "./dataset/TinyImagenet",
        prefix = "collections/tinyimagenet200/",
        mode=None,
        ...
        if mode in ["test"]:
            ...
            X_train_list, X_val_list = [], []
            for sid in range(10):
                collection_name = os.path.join(prefix,"tinyimagenet200_train_%s_rand1_cls20_task%d.json"%(exp_name,sid))
                with open(collection_name,"r") as f: X = json.load(f)
                y = [item['label'] for item in X]
                X_train, X_val, _, _ = train_test_split(X, y,stratify=y, test_size=0.05,random_state=seed)# seed for reproducibility
                X_train_list.append(X_train)
